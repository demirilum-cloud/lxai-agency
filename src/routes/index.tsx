  ...
  return (
    <div id="top" className="min-h-screen overflow-x-hidden bg-background text-foreground">
      ...
      <main>
        ...
        <section id="pricing" className="section-space"><div className="page-shell"><Reveal><SectionHeading eyebrow="05 / Investment" title="Scoped around value, not a one-size-fits-all package." copy="Every engagement starts with a focused conversation. You receive a clear recommendation, scope, timeline, and quote before we begin." /></Reveal>
          <Reveal className="mt-16"><div className="grid border border-border lg:grid-cols-[1.2fr_1fr]"><div className="p-7 sm:p-10 lg:p-14"><p className="section-label">Projects starting from</p><p className="mt-6 font-display text-6xl font-black tracking-normal sm:text-8xl">€750<span className="text-2xl text-muted-foreground">+</span></p><p className="mt-6 max-w-md leading-7 text-muted-foreground">Final pricing reflects complexity, integrations, content, and ongoing support. Larger automation and web projects are quoted individually.</p></div><div className="flex flex-col justify-between border-t border-border bg-surface p-7 sm:p-10 lg:border-l lg:border-t-0 lg:p-14"><div className="space-y-4">{["Free initial consultation", "Clear scope and delivery plan", "No hidden costs", "Optional ongoing support"].map(item => <div key={item} className="flex items-center gap-3 text-sm"><Check className="size-4" />{item}</div>)}</div><Button asChild size="xl" className="mt-12"><a href="#contact">Get a tailored quote <ArrowRight /></a></Button></div></div></Reveal>
        </div></section>
